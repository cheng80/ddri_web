...
- **절대 포함 금지**:
  - **이용권** (구매, 조회, 결제, 구독 등)
  - **사용자 my** (로그인, 회원가입, 프로필, 즐겨찾기, 마이페이지 등)
- **허용**: 대여소 조회, 대여 가능성 확인, 재배치 관리자용 목록·통계
- UI·API·DB 설계 시 위 항목이 스며들지 않도록 **반드시** 점검한다.
...
- **Chrome**웹으로 실행
- **반응형**: 모바일·태블릿·데스크탑 모두 지원. `flutter_screenutil` 사용.
- **반응형 일관성**: 구간별 레이아웃만 다르고, 색상·타이포·컴포넌트 스타일은 공통 Design Token으로 유지.
...
- **Controller**: GetX 상태 관리 (예: TodoListController, TagListController)
