...
4. **저장 위치**: `stitch_export/user/` 또는 `stitch_export/admin/` (페이지별 폴더)
...
- [ ] `stitch_export/user/` 또는 `stitch_export/admin/` (페이지별) 폴더에 스크린샷·HTML 보관
- [ ] `09_stitch_design_application_guide.md` (본 문서) 참고
- [ ] Primary 색상 `#00a857` → `ddri_web` ThemeData 반영
- [ ] 이용권·사용자 my(로그인·프로필·즐겨찾기) 등 **절대 금지** UI는 HTML에서 제외 후 적용
