...
- [x] 사용자/관리자 베타 안내 다이얼로그 및 `오늘은 다시 보지 않기` 처리 추가
- [x] 사용자/관리자 우측 상단 `BETA` 리본과 제목 옆 상태칩 추가
- [x] 사용자/관리자 목록·지도 섹션에 베타 범위 안내 문구 추가
- [x] 웹 릴리즈 빌드를 `/ddri/` 서브폴더 배포 기준으로 생성
- [x] `kpostal_plus` 웹 내부 geocoding 예외를 로컬 오버라이드로 우회
- [x] 사용자 웹 위치 요청을 단일 `getCurrentPosition()` 흐름으로 재정리
- [x] 사용자 베타 안내 다이얼로그를 위치 로딩 완료 후 표시하도록 조정
...
- [x] `/ddri/` 서브폴더 배포용 웹 release build 생성 (`flutter build web --release --base-href /ddri/`)
- [x] 배포 환경 mixed content 원인 확인: HTTPS 웹 + HTTP FastAPI(`:18000`) 직접 호출은 브라우저 정책상 차단
...
- [x] Selenium 기준 다중 뷰포트 지도 안정화 테스트
- [x] 베타 안내 다이얼로그, 제목 상태칩, `BETA` 리본 반영
- [x] 목록/지도에 베타 범위 보조 문구 반영
- [x] 웹에서 `FilledButton.icon` 렌더 assert 회피를 위해 검색 버튼을 일반 버튼 + Row 구조로 변경
- [x] 웹 위치 자동 호출을 다시 복원하고, 베타 안내 팝업은 위치 로딩 이후로 지연
- [ ] macOS/브라우저 위치 공급 실패(`kCLErrorLocationUnknown`) 재발 여부 최종 확인
...
1. 사용자 웹 위치 자동 호출/수동 호출 재발 여부 최종 확인
2. 관리자 페이지 태블릿/모바일/데스크탑 레이아웃 최종 점검
3. 사용자 지도 실제 수동 조작 기준 새 검색 결과 중심 이동 체감 최종 확인
4. 관리자 날씨/예외 폴백 상태 사용자 안내 방식 최종 점검
5. `prediction_logs` 중복 저장 방지 정책 확정
6. 베타 6개 스테이션 선정 기준과 원본 위치 문서화
7. 차기 운영용 마스터 로딩 원본과 해제 절차 확정
