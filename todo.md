...
- top6 임시 런타임 E2E, `prediction_logs` 저장 연결, 지도/드롭다운 주요 UI 안정화는 끝났고, 남은 핵심은 관리자 최종 레이아웃 마감과 중복 저장 정책 확정이다.
...
2. 사용자 검색 조건 변경 시 지도 중심 이동 체감 최종 확인
3. 관리자 지도/브레이크포인트 표시 안정성 최종 확인
4. 관리자 날씨/예외 폴백 상태 안내 방식 점검
...
```text
plan.md와 todo.md를 먼저 읽고 현재 상태를 요약한 뒤, 관리자 최종 레이아웃 점검과 prediction_logs 중복 저장 정책 정리부터 진행해줘.
```
